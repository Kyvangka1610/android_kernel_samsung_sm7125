...
export PATH="$HOME/toolchain/clang-r468909/bin:$PATH"
export LD_LIBRARY_PATH="$HOME/toolchain/clang-r468909/lib:$LD_LIBRARY_PATH"
export KBUILD_COMPILER_STRING="$($HOME/toolchain/clang-r468909/bin/clang --version | head -n 1 | perl -pe 's/\((?:http|git).*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//' -e 's/^.*clang/clang/')"
